...
export async function sendTrackerReminderEmails(app) {
    ...
    projects.forEach(project => {
        ...
        if (approvedSet.has(project)) return; // skip completed/approved items
...
export async function runAutoAssign(app, rerenderTracker, rerenderDashboard) {
    ...
    if (capacityPerDate <= 0) return notify("Reviewer pool must be larger than reviewers-per-proposal.", "error");
    ...
    // Mark overflow projects as unassigned with no date
